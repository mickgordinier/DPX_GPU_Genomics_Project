...
def readOutputSequences(filePath):
    ...
    # Safeguard in case the file does not end neatly
    lines = lines[0:-2]

    i = 0
    while i < len(lines):
        # Check if the line ends with "| 0"
        if lines[i].strip().endswith("| 0"):
            i += 1
            continue
        
        # Ensure there are enough remaining lines to extract a full block
        if i + 3 < len(lines):
            try:
                scoreOutputs.append(lines[i].split("|")[1].strip())
            except IndexError:
                print(f"Line {i} missing a score or improperly formatted: {lines[i]}")
                scoreOutputs.append("")  # Continue with a placeholder
        ...
        i += 4  # Move to the next block of 4 lines
    ...
    #       alignmentOutputs.append(lines[i + 2].replace('\n', ''))  # don't strip this
    #       queryOutputs.append(convert_to_dna(lines[i + 3].strip()))
    #       # print(f"Incomplete block starting at line {i}, ignoring remaining lines.")
    #       # break
...
def scrape_smith_waterman(page, reference, query):
    
  page.wait_for_selector("#sequence_1")
  page.wait_for_selector("#sequence_2")

  # Fill in the sequences
  page.fill("#sequence_1", query)
  page.fill("#sequence_2", reference)
...
def main():
  ...
  scoreOutputs, referenceOutputs, alignmentOutputs, queryOutputs = readOutputSequences("align-output-newest.txt")
  ...
  with sync_playwright() as p:
    ...
    # Open SW URL
    page.goto("https://rna.informatik.uni-freiburg.de/Teaching/index.jsp?toolName=Smith-Waterman")
    ...
    for i in range(len(input_references)):
      ...
      score, paths = scrape_smith_waterman(page, input_references[i], input_queries[i])
